        ...
        <div className={`bg-neutral-900/95 backdrop-blur-md rounded-2xl p-3 shadow-2xl transition-colors ${
            ...
            <div className="text-[11px] font-mono text-sky-400/90 flex items-center gap-1.5 truncate max-w-full">
              {selectedGroup ? (
                <>
                  ...
                  <span className="text-neutral-200 font-semibold truncate max-w-[130px] md:max-w-xs" title={selectedGroup.members.map((m) => m.name).join(', ')}>
                    {selectedGroup.members.map((m) => m.name).join(', ')} ({selectedGroup.size}p)
                  </span>
                  <span className="text-neutral-600">•</span>
                  <span className="text-amber-300 font-bold">Forward / Back</span>
                  <span className="text-neutral-400">cycles gate</span>
                  <span className="text-neutral-600">•</span>
                  <span className="text-sky-300 font-bold">Keys [1–8]</span>
                </>
              ) : (
                <>
                  <span className="text-emerald-300 font-semibold">Left Click</span>
                  <span className="text-neutral-400">calls queue</span>
                  <span className="text-neutral-600">•</span>
                  <span className="text-amber-300 font-bold">Forward / Back</span>
                  <span className="text-neutral-400">cycles gate (1–8)</span>
                  <span className="text-neutral-600">•</span>
                  <span className="text-sky-300 font-bold">Keys [1–8]</span>
                </>
              )}
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    ...
                    <button
                      ...
                      title={`Gate ${gateIdxA + 1} (${occA}/4: ${Math.min(2, occA)} boarding, ${Math.max(0, occA - 2)} in queue) • Assign with Key [${gateKeyLabels[gateIdxA]}]`}
                    ...
                    <button
                      ...
                      title={`Gate ${gateIdxB + 1} (${occB}/4: ${Math.min(2, occB)} boarding, ${Math.max(0, occB - 2)} in queue) • Assign with Key [${gateKeyLabels[gateIdxB]}]`}
                      className={`relative p-1.5 rounded-lg border flex flex-col items-center justify-center transition-all cursor-pointer select-none text-left w-full ${
                        isSelectedB
                          ? 'bg-sky-950/90 border-sky-400 text-sky-200 shadow-[0_0_14px_rgba(56,189,248,0.5)] ring-2 ring-sky-400'
                          : occB === 4
          ...
          <div className="mt-3 flex items-center justify-between gap-4">
            <div className="text-xs text-neutral-400 font-mono hidden sm:block">
              {selectedGroup ? (
                <span>
                  Assigning <strong className="text-white">{selectedGroup.size} guests</strong> to 1 Gate • <span className="text-sky-300 font-bold">1–8</span> Gate • <span className="text-amber-300 font-bold">Forward / Back</span> Cycle Gate • <span className="text-emerald-300 font-bold">Right Click</span> Let Group Go
                </span>
              ) : (
                <span>
                  Controls: <span className="text-emerald-300 font-bold">Left Click</span> Call Queue • <span className="text-sky-300 font-bold">1–8</span> Gate • <span className="text-amber-300 font-bold">Forward / Back</span> Cycle Gate (wrapping) • <span className="text-emerald-300 font-bold">Right Click / Enter</span> Let Group Go • <span className="text-neutral-200">[P]</span> Settings
                </span>
              )}
            </div>
